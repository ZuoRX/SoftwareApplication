...
write.csv(data_pp1,"result/data_pp.csv",row.names = F)

data_pp1<-read.csv("result/data_pp.csv") %>% 
  subset(pid!=779 & pid!=648 & pid!=470 & pid!=292 & pid!=122)

write.csv(data_pp1,"result/data_pp_update.csv",row.names = F)

#------------------------------#                                               
#-------3.描述性统计分析-------#
#------------------------------#
#（1）描述性统计分析
des<-describe(data_pp1) %>% 
...
write.csv(des,"result/data_pp_des.csv",row.names = F)
...
write.csv(cor1,"result/data_pp_cor.csv",row.names = F)
...
ggsave("result/data_pp_correlations.png",width=20, height=20,units="cm",dpi = 600)
...
fit<-glm.nb(commits~mdegree+mdegree2+mbetween+mbetween2+mclose+mclose2+
              mweight+diameter+networksize+networksize2+age,data=data_pp1)
...
fit_new<-glm.nb(commits~mdegree+mdegree2+mbetween+mbetween2+mclose+mclose2+
              mweight+diameter+networksize+networksize2+density+connect+age,data=data_pp1)
