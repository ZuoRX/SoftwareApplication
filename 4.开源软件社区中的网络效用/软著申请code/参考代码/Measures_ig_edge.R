...
for (i in 1:28402) { #28402		
		...
		for (year in 1999:2005) {
		  for (month in 0:0) {
		      filename <- paste("D:/OSS/finaldatfiles1010/OssMMNet", year, "_", month, "_", i, ".net", sep="")
					...
					if (file.exists(filename)) {
						...
						## caculate the weight suMMary information for a certain graph
						wei <- E(gig)$weight;
						weim <- mean(wei);
						weiv <- var(wei);
						weimx <- max(wei);
						...
						#wtc <- walktrap.coMMunity(gig);
						#memb <- coMMunity.to.membership(gig,wtc$merges,steps=12)
			      ...
			      print(result);      				
						...
						sql1 = paste( "update project_dyn1010 ",
							 	...
							 	" WHERE pid=", i, " and TP=", year, " and Type=", "'MM'", " and month=", month, sep="" );
            ...
            for (vi in 1:vcount(gig)) {
								sql23 = paste( "insert vertex1010 (pid, Type, TP, month, nodeid, loadve) values(",
									 i, ", ",  "'MM'", ", ", year, ", ", month, ", ", vi, ", ", V(gig)$loadve[vi], ")", sep="" );
								sql24 = paste( "update vertex1010 ",
									 	"set betweeness=", be[vi],
									 	" WHERE pid=", i, " and TP=", year, " and Type=", "'MM'", " and month=", month, " and nodeid=", vi, sep="" );
					  ...
					  wei <- get.edge.attribute(gig,"weight");
					  ebe <- edge.betweenness(gig);
					  
					  for (ei in 1:ecount(gig)) {
								sql28 = paste( "insert edge1010 (pid, Type, TP, month, edgeid, loadve) values(",
									 i, ", ",  "'MM'", ", ", year, ", ", month, ", ", ei, ", ", wei[ei], ")", sep="" );
								sql29 = paste( "update edge1010 ",
									 	"set betweeness=", ebe[ei],
									 	" WHERE pid=", i, " and TP=", year, " and Type=", "'MM'", " and month=", month, " and edgeid=", ei, sep="" );
									 	
								print(sql28);
								#print(sql24);
							  dbSendQuery(con, statement=sql28)
							  dbSendQuery(con, statement=sql29)
					  }
